...
docker run --rm -v "$($basePath):/var/src" mcr.microsoft.com/dotnet/core/sdk:3.1-alpine3.11 ash -c "dotnet publish -c Release /var/src/GMeet.csproj -o /var/src/bin/build && chown -R $($uid):$($gid) /var/src"
